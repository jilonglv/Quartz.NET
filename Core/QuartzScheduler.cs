...
namespace Quartz.Core
{
    ...
    public class QuartzScheduler : MarshalByRefObject, IRemotableQuartzScheduler
    {
        ...
        public QuartzScheduler(QuartzSchedulerResources resources, TimeSpan idleWaitTime) : this()
        {
            ...
            log.Info(CultureInfo.InvariantCulture, "Quartz Scheduler v.{0} created.", Version);
        }
        ...
        private class DelayedSchedulerStarter
        {
            ...
            public DelayedSchedulerStarter(QuartzScheduler scheduler, TimeSpan delay, NLog.Logger logger)
            {
        ...
        public virtual void Shutdown(bool waitForJobsToComplete)
        {
            ...
            log.Info(CultureInfo.InvariantCulture, "Scheduler {0} shutting down.", resources.GetUniqueIdentifier());
                ...
                foreach (IJobExecutionContext job in jobs)
                {
                    ...
                    if (jobInstance != null)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            log.Warn("Encountered error when interrupting job {0} during shutdown: {1}", job.JobDetail.Key, ex.ToString());
                        }
                    }
                }
            }
        ...
        public virtual DateTimeOffset ScheduleJob(IJobDetail jobDetail, ITrigger trigger)
        {
            ...
            if(jobDetail is JobDetailRemoteImpl)
            {
                var job= (JobDetailRemoteImpl)jobDetail;
                job.JobType = LoadType(job.JobTypeName);
            }
            if (trigger == null)
            {
        ...
        public virtual void NotifySchedulerListenersPausedTrigger(TriggerKey triggerKey)
        {
            ...
            foreach (ISchedulerListener sl in schedListeners)
            {
                ...
                {
                    log.Error(CultureInfo.InvariantCulture, "Error while notifying SchedulerListener of paused trigger. Trigger={0}", e, triggerKey);
                }
            }
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                {
                    log.Warn("disposing scheduler without waiting the currently running jobs (count = {0})", executingJobCount);
                }
                Shutdown(false);
            }
        }
