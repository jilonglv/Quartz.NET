                ...
                if (UseDBLocks)
                {
                    if (Delegate is SqlServerDelegate)
                    {
                        if (SelectWithLockSQL == null)
                        {
                            ...
                            Log.Info("Detected usage of SqlServerDelegate - defaulting 'selectWithLockSQL' to '" + DefaultLockSql + "'.", TablePrefix, "'" + InstanceName + "'");
                            SelectWithLockSQL = DefaultLockSql;
                        }
                    }
                    else if (Delegate is SQLiteDelegate)
                    {
                        const string DefaultLockSql = "SELECT * FROM {0}LOCKS WHERE " + ColumnSchedulerName + " = {1} AND LOCK_NAME = @lockName";
                        //const string DefaultLockSql = "SELECT * FROM {0}{1} WHERE {2} = {3} AND {4} = @lockName" + ColumnSchedulerName + " = {1} AND LOCK_NAME = @lockName";
                        Log.Info("Detected usage of SQLiteDelegate - defaulting 'selectWithLockSQL' to '" + DefaultLockSql + "'.", TablePrefix, "'" + InstanceName + "'");
                        SelectWithLockSQL = DefaultLockSql;
                    }
            ...
            return ExecuteInNonManagedTXLock(LockTriggerAccess,
                conn =>
                {
                    ...
                    foreach (IOperableTrigger trigger in triggers)
                    {
                        ...
                        {
                            log.Error("Caught job persistence exception: " + jpe.Message, jpe.ToString());
                            result = new TriggerFiredResult(jpe);
                        }
                        catch (Exception ex)
                        {
                            log.Error("Caught exception: " + ex.Message, ex.ToString());
