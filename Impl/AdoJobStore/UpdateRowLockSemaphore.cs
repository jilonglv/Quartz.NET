    ...
    public class UpdateLockRowSemaphore : DBSemaphore
    {
        ...
        protected override void ExecuteSQL(ConnectionAndTransactionHolder conn, string lockName, string expandedSQL, string expandedInsertSQL)
        {
            ...
            for (int i = 0; i < RetryCount; i++)
            {
                ...
                catch (Exception e)
                {
                    ...
                    if ((i + 1) == RetryCount)
                    {
                        Log.Debug("Lock '{0}' was not obtained by: {1}", lockName, Thread.CurrentThread.Name);
                    }
                    else
                    {
                        Log.Debug("Lock '{0}' was not obtained by: {1} - will try again.", lockName, Thread.CurrentThread.Name);
                    }
                    try
                    {
        ...
        private bool LockViaUpdate(ConnectionAndTransactionHolder conn, string lockName, string sql)
        {
            using (IDbCommand cmd = AdoUtil.PrepareCommand(conn, sql))
            {
                ...
                Log.Debug("Lock '{0}' is being obtained: {1}", lockName, Thread.CurrentThread.Name);
        ...
        {
            Log.Debug("Inserting new lock row for lock: '{0}' being obtained by thread: {1}", lockName, Thread.CurrentThread.Name);
            using (IDbCommand cmd = AdoUtil.PrepareCommand(conn, sql))
            {
                AdoUtil.AddCommandParameter(cmd, "lockName", lockName);
