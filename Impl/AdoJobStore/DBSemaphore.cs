...
namespace Quartz.Impl.AdoJobStore
{
    ...
    public abstract class DBSemaphore : StdAdoConstants, ISemaphore, ITablePrefixAware
    {
        ...
        public bool ObtainLock(DbMetadata metadata, ConnectionAndTransactionHolder conn, string lockName)
        {
            ...
            {
                Log.Debug("Lock '{0}' is desired by: {1}", lockName, Thread.CurrentThread.Name);
            }
            if (!IsLockOwner(lockName))
            {
                ...
                if (Log.IsDebugEnabled)
                {
                    Log.Debug("Lock '{0}' given to: {1}", lockName, Thread.CurrentThread.Name);
                }
            ...
            {
                Log.Debug("Lock '{0}' Is already owned by: {1}", lockName, Thread.CurrentThread.Name);
            }
        ...
        public void ReleaseLock(string lockName)
        {
            if (IsLockOwner(lockName))
            {
                if (Log.IsDebugEnabled)
                {
                    Log.Debug("Lock '{0}' returned by: {1}", lockName, Thread.CurrentThread.Name);
                }
            ...
            {
                Log.Warn("Lock '{0}' attempt to return by: {1} -- but not owner!",
                    new Exception("stack-trace of wrongful returner").ToString(),
                    lockName, 
