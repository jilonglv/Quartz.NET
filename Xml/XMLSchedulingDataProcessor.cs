...
namespace Quartz.Xml
{
    ...
    public class XMLSchedulingDataProcessor
    {
        ...
        public virtual void ProcessFile(string fileName, string systemId)
        {
            ...
            Log.Info(CultureInfo.InvariantCulture, "Parsing XML file: {0} with systemId: {1}", fileName, systemId);
        ...
        {
            Log.Info(CultureInfo.InvariantCulture, "Parsing XML from stream with systemId: {0}", systemId);
                    ...
                    while (triggersOfJob.Count > 0)
                    {
                        ...
                        if (dupeT != null)
                        {
                            if (OverWriteExistingData)
                            {
                                if (log.IsDebugEnabled)
                                {
                                    log.Debug("Rescheduling job: {0} with updated trigger: {1}", trigger.JobKey, trigger.Key);
                                }
                            }
                            ...
                            {
                                log.Warn("Possibly duplicately named ({0}) triggers in jobs xml file! ", trigger.Key);
                            }
                        ...
                        else
                        {
                            if (log.IsDebugEnabled)
                            {
                                log.Debug("Scheduling job: {0} with trigger: {1}", trigger.JobKey, trigger.Key);
                            }
                            ...
                            catch (ObjectAlreadyExistsException)
                            {
                                if (log.IsDebugEnabled)
                                {
                                    log.Debug("Adding trigger: {0} for job: {1} failed because the trigger already existed.  " 
            ...
            foreach (IMutableTrigger trigger in triggers)
            {
                ...
                if (dupeT != null)
                {
                    if (OverWriteExistingData)
                    {
                        if (log.IsDebugEnabled)
                        {
                            log.Debug("Rescheduling job: " + trigger.JobKey + " with updated trigger: " + trigger.Key);
                        }
                    }
                    else if (IgnoreDuplicates)
                    {
        ...
        private void DoRescheduleJob(IScheduler sched, IMutableTrigger trigger, ITrigger oldTrigger)
        {
            ...
            {
                Log.Debug("Using relative scheduling for trigger with key {0}", trigger.Key);
        ...
        protected void ExecutePreProcessCommands(IScheduler scheduler)
        {
            foreach (string group in jobGroupsToDelete)
            {
                ...
                else
                {
                    ...
                    {
                        log.Info("Deleting all jobs in group: {0}", group);
            ...
            foreach (string group in triggerGroupsToDelete)
            {
                ...
                else
                {
                    ...
                    {
                        log.Info("Deleting all triggers in group: {0}", group);
            ...
            foreach (TriggerKey key in triggersToDelete)
            {
                ...
                {
                    log.Info("Deleting trigger: {0}", key);
                    scheduler.UnscheduleJob(key);
                }
            }
        }
